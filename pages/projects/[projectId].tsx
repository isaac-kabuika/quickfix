...
function ProjectPage() {
  ...
  const handleBulkDelete = async () => {
    if (confirm('Are you sure you want to delete the selected bug(s)?')) {
      try {
        for (const bugId of selectedBugs) {
          await deleteBugReport(bugId)
        }
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white dark:bg-gray-800 shadow-md flex items-center justify-between p-2 px-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-2">
          <a href={project?.github_repo} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
          ...
          <div className="relative">
            <div className="flex items-center space-x-2">
              ...
              {(selectedBugs.size === 1 || selectedBugs.size > 1) && (
              ...
              <div id="action-buttons" className="flex items-center space-x-2">
                {selectedBugs.size === 1 && (
                  <>
                    ...
                    <button
                      onClick={() => handleBugDelete(Array.from(selectedBugs)[0])}
                      className="bg-white dark:bg-gray-700 text-red-600 dark:text-red-400 px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors border border-gray-300 dark:border-gray-600 flex items-center"
                    >
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <Link href="/dashboard" className="bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors border border-gray-300 dark:border-gray-600 flex items-center">
          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
